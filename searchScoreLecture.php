...
?>
<table class="table table-striped" style="width: 1024px; margin: auto;">
    <thead>
    <tr>
    ...
    if ($chknum != 0) {
        while ($row = mysqli_fetch_array($result)) {
            ...
            $sql = "SELECT * FROM score_lecture WHERE key_student = '$key_student' and key_set_lExam = '$key_exam'";
            $result_student = mysqli_query($database, $sql);
            $sqlfullscore = "SELECT * FROM set_lecture_exam WHERE key_set_lExam = '$key_exam'";
            $resultfullscore = mysqli_query($database, $sqlfullscore);
            $rowresultfullscore = mysqli_fetch_assoc($resultfullscore);
            $full_score= $rowresultfullscore['fullScore'];
            ...
            while ($row_student = mysqli_fetch_array($result_student)) {
                ...
                $student_name = $rowlecture['name_student'];
              ...
              onChange="updateScore(\'php/updateScore.php\',\'myDiv\',\'score_lecture\',\''.$key_student.'\',\''.$key_score_lecture.'\',this,\''.$current_score.'\',\''.$full_score.'\')" onkeyPress="updateScoreByEnter(event,\'php/updateScore.php\',\'myDiv\',\'score_lecture\',\''.$key_student.'\',\''.$key_score_lecture.'\',this,\''.$current_score.'\',\''.$full_score.'\')"></td>';
