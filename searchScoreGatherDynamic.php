...
<table class="table table-striped" style="width: 1024px; margin: auto;">
    ...
    if ($chknum != 0) {
        while ($row = mysqli_fetch_array($result)) {
            ...
            $sqlfullscore = "SELECT * FROM set_score_gather_dynamic WHERE key_set_gather = '$key_exam'";
            $resultfullscore = mysqli_query($database, $sqlfullscore);
            $rowfullscore = mysqli_fetch_assoc($resultfullscore);
            $full_score= $rowfullscore['fullscore'];
