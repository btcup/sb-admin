<?
//add to score gather
function addToGather()
{
    $host = "localhost";
    $user = "root";
    $pass = "";
    $dbname = "project_c";

    $database = mysqli_connect($host,$user,$pass,$dbname)or die(mysqli_error($objconnet));
//print_r($objconnet);
//$database = mysqli_select_db($objconnet,$dbname)or die("select db fial");
//print_r($database);
    mysqli_query($database,"SET NAMES UTF8");
    $sql = "SELECT * FROM student ORDER BY key_student DESC LIMIT 1";
$result_gather = mysqli_query($database,$sql);
					
while ($row_gather = mysqli_fetch_assoc($result_gather))
$key_student = $row_gather['key_student'];
					
$sql = "INSERT INTO score_gather (key_student) VALUES('$key_student')";
$result_gather = mysqli_query($database,$sql);
}
function addToGatherDynamic($key_set_gather,$key_year_subject)
{
    $host = "localhost";
    $user = "root";
    $pass = "";
    $dbname = "project_c";

    $database = mysqli_connect($host, $user, $pass, $dbname) or die(mysqli_error($objconnet));
//print_r($objconnet);
//$database = mysqli_select_db($objconnet,$dbname)or die("select db fial");
//print_r($database);
    mysqli_query($database, "SET NAMES UTF8");
    $sql = "SELECT * FROM sec_subject
			INNER JOIN student
			ON student.key_sec_subject = sec_subject.key_sec_subject
			WHERE sec_subject.key_year_subject = '$key_year_subject'";
    $result = mysqli_query($database,$sql);
    while($row = mysqli_fetch_array($result))
    {
        $key_student = $row['key_student'];
        $sql = "SELECT * FROM score_gather_dynamic WHERE key_student = '$key_student' and key_set_gather= '$key_set_gather'";
        $result_duplicate = mysqli_query($database,$sql);
        $chknum = mysqli_num_rows($result_duplicate);
        if($chknum == 0)
        {
            $sql = "INSERT INTO score_gather_dynamic (key_student,key_set_gather)
				VALUES('$key_student','$key_set_gather')";
            $result_lecture = mysqli_query($database,$sql);
        }
    }

}
?>