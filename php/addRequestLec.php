...
for($i = 1 ; $i<=$num ; $i++)
{
    ...
    if(isset($_POST[$cb_name]))
    {
        ...
        if($tx_value != "")
        {
            ...
            $sql_round = "SELECT DISTINCT date,time_start,time_end FROM round_exam_lec
            ...
            $sql_request = "SELECT * FROM request_lec WHERE key_student = '$key_student' and sub_semester = '$sub_semester'";
            ...
            if($chknum == 0)
            {
                $sql = "INSERT INTO request_lec (key_student,date,time_start,time_end,sub_semester,reason,numofexam)
