...
while ($row_request = mysqli_fetch_array($result_request)) {
    ...
    $re_sql_round = "SELECT * FROM round_exam_lec WHERE key_year_subject = '$key_year_subject' and sub_semester = '$sub_semester'
    ...
    $sql_round = "SELECT * FROM round_exam_lec WHERE key_year_subject = '$key_year_subject' and sub_semester = '$sub_semester'";
    ...
    while ($row_round = mysqli_fetch_array($result_round)) {
        ...
        if ($re_key_round != $key_round && $remain > 0 && $re_status != "w" && $re_status != "W") {
            ...
            $sql_chk_dup = "SELECT * FROM generate_password_lec
							INNER JOIN round_exam_lec ON round_exam_lec.key_year_subject = '$key_year_subject'
							AND round_exam_lec.sub_semester = '$sub_semester'
							WHERE generate_password_lec.key_student = '$re_key_student' AND generate_password_lec.round_room = round_exam_lec.key_round";
            $result_chk_dup = mysqli_query($database, $sql_chk_dup);
            $chk_dup = mysqli_num_rows($result_chk_dup);
            if ($chk_dup == 0) {
                $password = randomPassword();
                $sql_add = "INSERT INTO generate_password_lec (key_student,round_room,password)
                ...
                //update remianing
                $new_remain = $remain - 1;
                $sql_update_remain = "UPDATE round_exam_lec SET remaining = '$new_remain' WHERE key_round = '$key_round'";
...
$student++;
    $sql_round = "SELECT * FROM round_exam_lec WHERE key_year_subject = '$key_year_subject' and sub_semester = '$sub_semester'and numofexam = '$numofexam' and round_number = '$round'";
    $result_round = mysqli_query($database, $sql_round);
    while ($row_round = mysqli_fetch_array($result_round)) {
        $key_round = $row_round['key_round'];
        $remain = $row_round['remaining'];
        if ($remain > 0) {
            ...
            $sql_chk_dup = "SELECT * FROM generate_password_lec
							INNER JOIN round_exam_lec ON round_exam_lec.key_year_subject = '$key_year_subject' 
							AND round_exam_lec.sub_semester = '$sub_semester' and round_exam_lec.numofexam = '$numofexam' and round_number = '$round'
							WHERE generate_password_lec.key_student = '$key_student' AND generate_password_lec.round_room = round_exam_lec.key_round";
            $result_chk_dup = mysqli_query($database, $sql_chk_dup);
            $chk_dup = mysqli_num_rows($result_chk_dup);
...
echo "รอบ".$round." "."ครั้งที่".$numofexam;
echo "มีนิสิต: " . $student . " คน";
echo "เข้า: " . $ch . " คน";
echo "เข้าถึง: " . $ch2 . " คน";
